<{if $categoryRows > 0}>
    <div class="outer">
        <form name="select" action="category.php?op=" method="POST"
              onsubmit="if(window.document.select.op.value =='') {return false;} else if (window.document.select.op.value =='delete') {return deleteSubmitValid('categoryId[]');} else if (isOneChecked('categoryId[]')) {return true;} else {alert('<{$smarty.const._AM_CATEGORY_SELECTED_ERROR}>'); return false;}">
